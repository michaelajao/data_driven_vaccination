...
def wavelet_denoise(data, wavelet='db1', level=1):
    ...
    return denoised_data[:len(data)]  # Ensure the length matches the original data

def wavelet_decompose(data, wavelet='db1', level=1):
    """Apply wavelet transform to decompose data."""
...
def plot_wavelet_decomposition(data, date, title, wavelet='db1', level=1):
    coeff = wavelet_decompose(data, wavelet, level)
    fig, axs = plt.subplots(len(coeff), 1, figsize=(12, 8))
    fig.suptitle(title, fontsize=16, weight='bold')

    for i, ax in enumerate(axs):
        # Ensure valid levels for upcoef
        level_val = max(level - i, 1)
        ax.plot(date, pywt.upcoef('a' if i == 0 else 'd', coeff[i], wavelet, level=level_val))
        ax.set_ylabel(f'Level {i}', fontsize=14, weight='bold')

    plt.xlabel('Date', fontsize=14, weight='bold')
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig(f"../../reports/figures/{title.replace(' ', '_').lower()}.pdf")
    plt.savefig(f"../../reports/figures/{title.replace(' ', '_').lower()}.png")  # Save as PNG
    plt.show()
...
# Plot wavelet decomposition of admissions and critical care data
plot_wavelet_decomposition(data["daily_hospitalized"], data["date"], "Wavelet Decomposition of Admissions Data")
plot_wavelet_decomposition(data["covidOccupiedMVBeds"], data["date"], "Wavelet Decomposition of Critical Care Data")
...
def plot_outputs(model, t, parameter_net, data, device, scaler):
    ...
    colors = ["purple", "red", "orange", "blue", "green", "violet"]  # Use a list for colors in case you want to customize further
