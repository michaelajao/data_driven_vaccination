...
def einn_loss(model_output, tensor_data, parameters, t, constants):
    ...
    loss = (data_weight * data_loss) + (residual_weight * residual_loss) + (initial_weight * initial_cost)
    return loss
...
model = EpiNet(num_layers=6, hidden_neurons=10, output_size=8).to(device)
